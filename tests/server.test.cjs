...
test('missing grounding sources are rejected without another provider call',async()=>{
...
test('opaque Gemini grounding redirect is used instead of rejecting or exposing an unverified URL',async()=>{
 const redirect='https://vertexaisearch.cloud.google.com/grounding-api-redirect/opaque-token';
 const cookie=(await invoke(session,req('POST',{gemini:keyA,preferred:'gemini'}))).headers['Set-Cookie'].split(';')[0];
 global.fetch=async()=>({ok:true,status:200,headers:{get:()=>null},json:async()=>({
   candidates:[{finishReason:'STOP',content:{parts:[{text:JSON.stringify({items:[item]})}]},groundingMetadata:{groundingChunks:[{web:{uri:redirect,title:'검증된 출처'}}]}}],
   usageMetadata:{totalTokenCount:50}
 })});
 const result=await invoke(news,req('POST',{keyword:'AI',days:3},cookie));
 assert.equal(result.statusCode,200);
 assert.equal(result.data.items[0].url,redirect);
 assert.notEqual(result.data.items[0].url,item.url);
});
