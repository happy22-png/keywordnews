...
module.exports=async function(req,res) {
  ...
  for(const provider of order) {
    ...
    try {
      ...
      if(error.status!==429) return s.send(res,error.status===401 || error.status===403?422:502,{error:error instanceof providers.ProviderError?error.message:'뉴스 처리에 실패했습니다.',attempts,provider});
