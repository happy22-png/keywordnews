...
async function fetchNews(provider,key,keyword,days) {
  ...
  const items=parsedItems.slice(0,4).filter(item=>sources.some(s=>s.url===safeUrl(item.url))).map(item=>({title:item.title.slice(0,200),snippet:item.snippet.slice(0,300),publisher:item.publisher.slice(0,100),date:item.date.slice(0,80),url:safeUrl(item.url)}));
  if(parsedItems.length && !items.length) throw new ProviderError(502,'검색 출처와 연결된 기사를 확인하지 못했습니다.');
